/**
 * SM-UPGRADE — Blog Post Transformation Script
 *
 * Converts all 50 blog posts from the old design system to the current
 * SM-UPGRADE token-based card system. Preserves all written content.
 *
 * Usage: node scripts/transform-blog.js
 */
...
// ─── HELPER: Strip old CSS class names from body content ──────────────────

function stripOldClasses(html) {
  return html
    .replace(/\sclass="blog-p"/g, '')
    .replace(/\sclass='blog-p'/g, '')
    .replace(/\sclass="blog-h2"/g, '')
    .replace(/\sclass='blog-h2'/g, '')
    .replace(/\sclass="blog-h3"/g, '')
    .replace(/\sclass='blog-h3'/g, '')
    .replace(/\sclass="blog-ul"/g, '')
    .replace(/\sclass='blog-ul'/g, '')
    .replace(/\sclass="blog-li"/g, '')
    .replace(/\sclass='blog-li'/g, '')
    .replace(/\sclass="blog-blockquote"/g, '')
    .replace(/\sclass='blog-blockquote'/g, '');
}
...
function extractBetween(text, startMarker, endMarker) {
  ...
  if (end === -1) return text.slice(afterStart).trim();
  return text.slice(afterStart, end).trim();
}

function extractBetweenExclude(text, startMarker, endMarker) {
  const start = text.indexOf(startMarker);
  if (start === -1) return '';
  const afterStart = start + startMarker.length;
  const end = text.indexOf(endMarker, afterStart);
...
function parsePost(filePath) {
  ...
  const cleanTitle = title.replace(/ — SMK Web Design$/, '').trim();
  ...
  // Category
  const catMatch = html.match(/<span class="blog-post-cat">([^<]+)<\/span>/);
  const category = catMatch ? catMatch[1].trim() : '';
  
  // H1 title
  const h1Match = html.match(/<h1 class="blog-post-title">([^<]+)<\/h1>/);
  const h1 = h1Match ? h1Match[1].trim() : cleanTitle;
  
  // Date and read time from meta
  const metaContent = extractBetween(html, '<div class="blog-post-meta">', '</div>');
  const dateMatch = metaContent.match(/(\d+\s+\w+\s+\d{4})/);
  const date = dateMatch ? dateMatch[1] : '';
  const readTimeMatch = metaContent.match(/(\d+\s*min\s*read)/);
  const readTime = readTimeMatch ? readTimeMatch[1] : '';
  
  // Body content
  const bodyMatch = html.match(/<div class="blog-post-content">([\s\S]*?)<\/div>\s*\n\s*<div class="blog-tags">/);
  let body = bodyMatch ? bodyMatch[1].trim() : '';
  body = stripOldClasses(body);
  
  // Tags
  const tagsMatch = html.match(/<div class="blog-tags">([\s\S]*?)<\/div>/);
  let tagsHtml = tagsMatch ? tagsMatch[1].trim() : '';
  // Convert old tag spans to new format
  tagsHtml = tagsHtml.replace(/<span class="blog-tag">([^<]+)<\/span>/g, '<span class="tech-pill">$1</span>');
  
  // Related articles
  const relatedMatch = html.match(/<section class="blog-related">([\s\S]*?)<\/section>/);
  let relatedHtml = relatedMatch ? relatedMatch[1].trim() : '';
  // Restyle related cards
  relatedHtml = relatedHtml
    .replace(/<h2 class="blog-related-title">([^<]+)<\/h2>/, '')
    .replace(/<div class="blog-related-grid">/, '<div class="blog-related-grid" style="display:grid;grid-template-columns:1fr;gap:var(--space-lg);">')
    .replace(/class="blog-card"/g, 'class="stack-card card" data-tilt style="text-decoration:none;display:block;"')
    .replace(/class="blog-card-body"/g, '')
    .replace(/class="blog-card-meta"/g, 'style="display:flex;gap:var(--space-md);font-size:var(--label-sm);color:var(--white-muted);margin-bottom:var(--space-sm);"')
    .replace(/class="blog-card-cat"/g, '')
    .replace(/class="blog-card-date"/g, '')
    .replace(/class="blog-card-title"/g, 'style="font-family:var(--font-display);font-weight:600;font-size:var(--body-lg);color:var(--white);margin-bottom:var(--space-sm);"')
    .replace(/class="blog-card-desc"/g, 'style="font-size:var(--body-sm);color:var(--white-muted);line-height:var(--leading-relaxed);"')
    .replace(/class="blog-card-footer"/g, 'style="display:none;"');
  
  return { title, cleanTitle, description, canonical, category, h1, date, readTime, body, tagsHtml, relatedHtml };
}

// ─── BUILD NEW POST HTML ─────────────────────────────────────────────────

function buildPostHTML(data) {
  const relatedSection = data.relatedHtml
    ? `
    <div class="section-card card" data-tilt data-morph data-holo>
  ...
  const tagsSection = data.tagsHtml
    ? `
    <div class="section-card card" data-tilt data-morph data-holo>
  ...
  return `${HEAD_OPEN}
  <title>${data.cleanTitle} | SMK Web Design</title>
  <meta name="description" content="${data.description}">
  <link rel="canonical" href="${data.canonical}">
  <meta property="og:type" content="article">
  <meta property="og:url" content="${data.canonical}">
  <meta property="og:title" content="${data.cleanTitle} | SMK Web Design">
  ...
  <meta name="twitter:title" content="${data.cleanTitle} | SMK Web Design">
  ...
  <meta name="robots" content="index, follow, max-image-preview:large">${STYLES}
${COSMIC_BG}
${NAV}
      ...
      <div class="container">
        ...
        <div class="hero__card card" data-tilt data-magnetic data-holo>
          <div style="display:flex;align-items:center;gap:var(--space-sm);margin-bottom:var(--space-md);">
            <span class="badge"><span class="badge__dot"></span>${data.category}</span>
          </div>
          <h1 class="hero__title tilt-layer--lg" data-reveal>${data.h1}</h1>
          <div class="hero__desc tilt-layer" style="font-size:var(--body-sm);color:var(--white-muted);margin-bottom:0;">${data.date}${data.readTime ? ' · ' + data.readTime : ''}</div>
        </div>

        <div class="section-card card" data-tilt data-morph data-holo style="margin-bottom:var(--space-xl);">
          <div class="blog-content" style="font-size:var(--body-md);color:var(--white-dim);line-height:var(--leading-relaxed);">
            ${data.body}
          </div>
        </div>
        ...
        <div style="text-align:center;margin-top:var(--space-xl);margin-bottom:var(--space-3xl);">
          <a href="/blog/" class="btn btn--ghost btn--lg">&larr; Back to Blog</a>
        </div>
...
function processAllPosts() {
  ...
  const postDirs = entries.filter(e => e.isDirectory() && e.name !== 'assets');
  ...
  for (const dir of postDirs) {
    ...
    if (!fs.existsSync(indexPath)) {
      failed.push(`${dir.name}/ (no index.html)`);
      continue;
    }

    try {
      const data = parsePost(indexPath);
      const newHtml = buildPostHTML(data);
...
console.log('SM-UPGRADE — Blog Post Transformation');
console.log('====================================\n');
console.log('Transforming post pages...');
const result = processAllPosts();
console.log(`\nDone: ${result.success} transformed, ${result.failed.length} failed`);
if (result.failed.length) {
  console.log('Failed:');
  result.failed.forEach(f => console.log(`  - ${f}`));
}
console.log('\nNow rebuild blog/index.html manually or run the index builder.');
